...
fun main() {
    ...
    if ((!inputDamage && (inputSizeOfCrew in minSizeOfCrew until recommendedSizeOfCrew) &&
                inputProvision > provision && (inputWeatherConditions || !inputWeatherConditions)) ||
        (inputDamage && inputSizeOfCrew == recommendedSizeOfCrew && inputProvision > provision &&
                inputWeatherConditions)
    ) println("Все условия соблюдены, корабль может отправляться в рейс.")
    else println("Условия отправки в рейс не соблюдены. Корабль отплыть не может.")
}